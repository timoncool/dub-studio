...
2. Дважды кликни **`run.bat`**. При первом запуске он сам всё установит (встраиваемый Python, CUDA-колёса под твою GPU, движок, собранный UI) и откроет редактор в браузере. Перетащи видео — модели докачаются автоматически при первом использовании.

Вот и всё — один клик, без ручных шагов. Цель — NVIDIA RTX 20xx–50xx (CUDA 12.8), ~12 ГБ VRAM. Всё живёт в папке приложения, система не засоряется. Запуск из исходников для разработки — см. [PACKAGING.md](PACKAGING.md).
