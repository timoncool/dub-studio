...
## Быстрый старт (Windows, NVIDIA GPU)

Установка прямо из репозитория - один `clone`, один двойной клик. Нужны [Git](https://git-scm.com/download/win) и NVIDIA GPU; всё остальное (Python, CUDA, Node, ffmpeg) скачается само.
...
Теперь дважды кликни **`run.bat`** (или запусти из терминала). **Первый запуск сам всё установит в эту папку** - встраиваемый Python 3.11, CUDA 12.8-колёса, движок, ffmpeg и Node, базовый голос-пак и собранный UI (он сам вызовет `install.bat`) - и откроет редактор на **http://127.0.0.1:8765**. Перетащи видео; модели (Gemma-4 GGUF, Parakeet, Qwen3-TTS, Sortformer) докачаются автоматически при первом использовании.

Вот и всё - без ручной настройки Python/CUDA/Node. Цель - NVIDIA RTX 20xx-50xx (CUDA 12.8), ~12 ГБ VRAM. Всё живёт внутри склонированной папки, систему не трогает. **Обновление потом** - `git pull`, затем **`update.bat`**. Детали сборки и упаковки: [PACKAGING.md](PACKAGING.md).

> **Хочешь без git, в один клик `.zip`?** Портативный `DubStudio_*.zip` появится в [Релизах](https://github.com/timoncool/dub-studio/releases), как только выйдет первый собранный билд - пока не опубликован, так что клонируй репозиторий.
