...
export type SubStyle = {
  ...
  n_lines?: number | null; align: string; size_px?: number | null; outline_w?: number | null;
};
export type Segment = {
  id: string; start: number; end: number; speaker?: string | null;
  src_text: string; tgt_text: string; voice?: string | null; dirty: boolean;
};
export type BlurBox = { x: number; y: number; w: number; h: number; t0: number; t1: number; hidden?: boolean };
export type Title = {
  ...
  outline_w?: number | null; uppercase?: boolean;
};
export type Project = {
