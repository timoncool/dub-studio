...
export default function PreviewCanvas({ pid, project, scrub, rendered, onChanged }: Props) {
  ...
  const previewSrc = rendered ? api.outputUrl(pid) : api.previewUrl(pid, scrub);
  ...
  async function patch(edit: Record<string, unknown>) {
    setBusy(true);
    try { await api.patch(pid, edit); onChanged(); } finally { setBusy(false); }
  }
  ...
  return (
    <div ref={wrap} className="relative w-full h-full grid place-items-center bg-black/50">
        ...
        {!rendered && disp.w > 0 && (
          <Stage width={disp.w} height={disp.h} className="absolute inset-0"
            ...
            <Layer>
              ...
              <Rect x={0} y={subY * sy - 14} width={disp.w} height={28} fill="rgba(124,92,255,0.18)"
                    stroke="#7c5cff" dash={[6, 4]} draggable
              ...
              {blurs.map((b, i) => (
                <Rect key={i} ref={(n) => { if (n) boxRefs.current[i] = n; }}
                      ...
                      fill={sel === i ? "rgba(0,212,184,0.15)" : "rgba(255,255,255,0.06)"}
                      stroke={sel === i ? "#00d4b8" : "#ffffff80"} strokeWidth={1} draggable
              ...
              {guide != null && <Line points={[guide, 0, guide, disp.h]} stroke="#00d4b8" dash={[4, 4]} />}
